...
import { RiAddLine, RiArrowLeftLine, RiArrowRightLine, RiCloseLine, RiDeleteBinLine, RiSearchLine, RiTeamLine, RiUserAddLine } from "@remixicon/react"
      ...
      <div ref={contentRef} className="flex-1 min-h-0 flex flex-col">
        {showIframe ? (
          <>
            ...
            <div className="flex-shrink-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 px-6 py-4 shadow-lg">
              <div className="flex items-center justify-between max-w-3xl mx-auto">
                <Button variant="ghost" onClick={handleBack}>
                  <RiArrowLeftLine className="size-4 mr-2" />
                  {iframeStep === 1 ? 'Back to teams' : 'Previous'}
                </Button>
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    variant="primary"
                    onClick={handleIframeNext}
                    disabled={iframeStep >= iframeTotalSteps}
                  >
                    {iframeStep >= iframeTotalSteps ? 'Complete' : 'Next'}
                    {iframeStep < iframeTotalSteps && <RiArrowRightLine className="size-4 ml-2" />}
                  </Button>
                </div>
              </div>
            </div>
