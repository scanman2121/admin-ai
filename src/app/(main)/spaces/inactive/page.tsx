...
export default function SpacesInactive() {
    const pathname = usePathname()
    const router = useRouter()
    const [data, setData] = useState<typeof allSpacesData>([])

    // Filter for inactive spaces only
    useEffect(() => {
        const inactiveSpaces = allSpacesData.filter(space => space.status === "Inactive")
        setData(inactiveSpaces)
    }, [])
